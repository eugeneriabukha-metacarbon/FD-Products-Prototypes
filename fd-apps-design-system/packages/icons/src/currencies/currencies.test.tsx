...
describe("currencies icons", () => {
  it("exports the expected 3 components", () => {
  ...
  it("size sets width/height", () => {
    const { container } = render(<icons.USDCIcon size={40} />);
  ...
  it("title yields role=img + <title>", () => {
    const { container, getByText } = render(<icons.USDCIcon title="USDC" />);
    expect(container.querySelector("svg")).toHaveAttribute("role", "img");
    expect(getByText("USDC").tagName.toLowerCase()).toBe("title");
  });
